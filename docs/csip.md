...
CSIP V1.2 section 10 mandates a subset of IEEE 2030.5 function sets. This table
...
- Empty Subject (LFDI is the first 160 bits of `SHA-256(DER-cert)`;
  SFDI is derived from LFDI per section 6.3)

### Device-type policy OID (section 6.11.7.2)
...
CSIP V1.2 section 6.2 mandates that every device cert carry a critical Subject
...
certificate (section 6.3), so the PEN bytes are part of the input. The PEN
...
an allowlist, no per-vendor handling. CSIP section 6.2 mandates the SAN is
...
- `make run-testdevice` - server bound to `127.0.0.1:8888` with the
  ...
  CSIP section 6.11-compliant. `TESTDEVICE_ADDR=host:port` overrides the bind.
