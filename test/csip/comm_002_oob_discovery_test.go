...
//
// CSIP V1.2 Section 5.2 - Out-of-Band Discovery. Asserts /dcap GET succeeds
...
//
// Procedure (V1.2 Section 5.2 - Out-of-Band Discovery):
...
// TestCOMM_002_OOBDiscovery proves the CSIP V1.2 Section 5.2 Out-of-Band
...
func TestCOMM_002_OOBDiscovery(t *testing.T) {
	...
	if dcap.EndDeviceListLink == nil || dcap.EndDeviceListLink.Href == "" {
		t.Errorf("EndDeviceListLink.Href is empty; CSIP V1.2 Section 5.2 requires it for per-device function-set walking")
	}
	if dcap.TimeLink == nil || dcap.TimeLink.Href == "" {
		t.Errorf("TimeLink.Href is empty; CSIP V1.2 Section 5.2 requires it (paired with CORE-005 time-quality assertion)")
	}
	if dcap.SelfDeviceLink == nil || dcap.SelfDeviceLink.Href == "" {
		t.Errorf("SelfDeviceLink.Href is empty; CSIP V1.2 Section 5.2 requires it for server-identity discovery")
	}
}
