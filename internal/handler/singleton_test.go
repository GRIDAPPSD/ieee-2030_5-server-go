...
func TestSingletonGetReturnsDefault(t *testing.T) {
	store := memory.NewScopedStore[sep2.DERCapability]()

	h := handler.HandleSingletonGetPut[sep2.DERCapability](
		store,
...
func TestSingletonPutThenGet(t *testing.T) {
	store := memory.NewScopedStore[sep2.DERSettings]()

	h := handler.HandleSingletonGetPut[sep2.DERSettings](
		store,
		func(r *http.Request) string { return "edev1/der1" },
...
func TestSingletonPutUpdateExisting(t *testing.T) {
	store := memory.NewScopedStore[sep2.DERStatus]()

	h := handler.HandleSingletonGetPut[sep2.DERStatus](
		store,
		func(r *http.Request) string { return "edev1/der1" },
