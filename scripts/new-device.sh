...
#   SERIAL         hardware serial for the CSIP §6.2 HardwareModuleName SAN
#
# Optional env:
...
if [[ -z "$SERIAL" ]]; then
  err "SERIAL=<hw-serial> is required (CSIP §6.2 HardwareModuleName SAN)"
  example
  exit 2
fi
...
# DEVICE_TYPE is the IEEE 2030.5 §6.11.7.2 device-type policy index.
case "$DEVICE_TYPE" in
  1|2|3) ;;
  *)
    err "DEVICE_TYPE must be 1 (Generic), 2 (Mobile), or 3 (PostMfg); got: ${DEVICE_TYPE}"
    err "See docs/csip.md §6.11.7.2 for the policy meanings."
    exit 2
    ;;
esac
