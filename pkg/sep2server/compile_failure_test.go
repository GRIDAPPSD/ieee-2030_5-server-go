...
// wantMutationCall is the exact call testdata/readerstorescannotmutate/main.go
...
// TestReaderStoresCannotMutate is the compile-failure proof for criterion 2
// of issue #343: ReaderStores' fields are typed as reader interfaces, so a
// caller holding Server.ReaderStores() cannot compile a call that mutates
// the store. testdata's readerstorescannotmutate program textually attempts
// a Create through the read handle; if ReaderStores.MirrorUsagePoints ever
...
// issue #367.
func TestReaderStoresCannotMutate(t *testing.T) {
	dir, err := filepath.Abs(filepath.Join("testdata", "readerstorescannotmutate"))
	if err != nil {
	...
	if !strings.Contains(string(src), wantMutationCall) {
		t.Fatalf("testdata/readerstorescannotmutate/main.go no longer attempts %q: this test would pass without proving anything", wantMutationCall)
	}
	...
	if err == nil {
		t.Fatalf("readerstorescannotmutate compiled; ReaderStores.MirrorUsagePoints admits Create:\n%s", out)
	}
