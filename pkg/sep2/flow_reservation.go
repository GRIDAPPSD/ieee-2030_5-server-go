...
func (f FlowReservationRequest) Copy() FlowReservationRequest {
	c := f
	if f.Version != nil { v := *f.Version; c.Version = &v }
	if f.DurationRequested != nil { v := *f.DurationRequested; c.DurationRequested = &v }
	if f.EnergyRequested != nil { v := *f.EnergyRequested; c.EnergyRequested = &v }
	if f.IntervalRequested != nil { v := *f.IntervalRequested; c.IntervalRequested = &v }
	if f.PowerRequested != nil { v := *f.PowerRequested; c.PowerRequested = &v }
	if f.RequestStatus != nil { v := *f.RequestStatus; c.RequestStatus = &v }
	return c
}
...
func (f FlowReservationResponse) Copy() FlowReservationResponse {
	...
	if f.PowerAvailable != nil { v := *f.PowerAvailable; c.PowerAvailable = &v }
	return c
}
