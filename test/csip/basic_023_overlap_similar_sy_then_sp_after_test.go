// CSIP V1.2 §8.23 — Overlap event prioritization, similar opMod, SP fully inside SY.
//
...
// mid-SY and ends before SY ends. Per §10.10 SP still wins inside its
// own window (lower primacy); SY re-applies outside SP's window.
//
// V1.2 procedure step → assertion mapping (per V1.2 §8.23):
//
...
//	Step 6 (SP window fully contained in SY; SP wins §10.10)        ──► assertOverlapResolution
//
...
// TestBASIC_023_OverlapSimilarSYThenSPAfter implements CSIP V1.2 §8.23.
...
func runBASIC023(t *testing.T, extraOpts []csiptest.BootOption) {
	...
	// §10.10: SP wins on primacy inside the contained window; SY
