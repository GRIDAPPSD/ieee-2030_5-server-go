...
// in parallel (they share package state in coresep2time's clockOffsetNanos).
...
// asserting that the package-level nowFunc seam in coresep2time reflects the
// mutation handler's offset; the auth chain is exercised by its own tests
// (internal/auth/...).
