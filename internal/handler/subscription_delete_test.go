...
// recordingNotifier is the test double for SubscriberNotifier. It records
// every call so the test can assert which subscription was notified.
type recordingNotifier struct {
	mu     sync.Mutex
	calls  []sep2.Subscription
	failOn map[string]error // map subID → error to return; nil = success
}
...
func TestHandleDeleteSubscription_NotifiesBeforeDelete(t *testing.T) {
	...
	mux.HandleFunc("DELETE /edev/{id}/sub/{subId}",
		handler.HandleDeleteSubscription(s, notifier))
...
// that a notifier returning an error does NOT change the 204 status —
// the final Notification is best-effort per the IEEE-100 design.
...
// remains backward-compatible: nil notifier means "skip the final
...
func TestHandleDeleteSubscription_NilNotifierAllowed(t *testing.T) {
	...
	mux := http.NewServeMux()
	mux.HandleFunc("DELETE /edev/{id}/sub/{subId}",
		handler.HandleDeleteSubscription(s, nil))
...
func TestHandleDeleteSubscription_Integration(t *testing.T) {
	...
	mux := http.NewServeMux()
	mux.HandleFunc("DELETE /edev/{id}/sub/{subId}",
		handler.HandleDeleteSubscription(s, mgr))
