...
- `store.ErrInvalidManagementPair`, returned when a management pair has an empty or non-canonical
  ...
  ([#354](https://github.com/GRIDAPPSD/ieee-2030_5-server-go/issues/354))
- A log line for every ownership refusal on `/edev/{id}` (route, caller LFDI, requested id, reason
  class), at most 20 per minute per router.
  ([#354](https://github.com/GRIDAPPSD/ieee-2030_5-server-go/issues/354))
...
- **Breaking for embedders and clients.** Every route under `/edev/{id}` now answers only the device
  ...
  `403` (not owner or manager, no identity, or a record with no LFDI) or `404` (no such EndDevice);
  ...
  lists only the caller's own and managed EndDevices, with `all` and `results` counting that set.
...
- A device holding a CA-signed client certificate can no longer read, change, or delete another
  device's EndDevice or the resources under it, and `GET /edev` no longer discloses every device's
  LFDI and SFDI. ([#354](https://github.com/GRIDAPPSD/ieee-2030_5-server-go/issues/354))
